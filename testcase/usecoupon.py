...
#读取配置文件参数
readcfg= readconfig_ex.readconfig('usecoupon')
...
class order(SequentialTaskSet):
    ...
    def suborder(self):
        ...
        orderdata=statmentorder_data
        # print(orderdata)
        self.headers=self.data['heads']
        if bf=='true':
            all_locusts_spawned.wait()  # 集合点等待并发，限制在所有用户准备完成前处于等待状态
            #print('------设置了集合点，并发执行------')
        else:
            pass
        '''#单用户循环,另一种方法通过参数序列索引取数进行循环使用
        orderdata = statmentorder_data[self.index]
        self.index = (self.index + 1) % len(statmentorder_data)
        '''
        nowtime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
        with self.client.post(path=statmentorder_url, data=json.dumps(orderdata),headers=self.headers,catch_response=True,verify=False) as checkresponse:
